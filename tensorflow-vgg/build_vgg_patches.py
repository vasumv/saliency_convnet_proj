...
if __name__ == "__main__":
    ...
    with tf.Session(config=tf.ConfigProto()) as sess:
        ...
        train_op = tf.train.RMSPropOptimizer(learning_rate).minimize(loss, global_step=batch_cnt)
        ...
        for epoch in range(num_epochs): 
            ...
            sub_length = len(sub_training_paths) - (len(sub_training_paths) % batch_size / 10)
            for i in range(0, sub_length, batch_size / 10):
                patches = []
                patch_ids = []
                for img_id in sub_training_paths[i:i+batch_size / 10]:
                    image = plt.imread(datapath + Path(image_paths[img_id]))
                    centers = np.load(samplepath + image_paths[img_id].split(".")[0] + ".npy").T
                    patches.extend(create_patches(image, centers, 80))
                    patch_ids.extend([img_id] * len(centers))
                if len(patches) == 0:
                    continue
                ...
                with open("build_vgg_log_patches.txt", "a") as f:
            ...
            sum_cost = 0.0
            sum_accuracy = 0.0
            valid_length = len(validation) - (len(validation) % batch_size / 10)
            for i in range(0, valid_length, batch_size / 10):
                patches = []
                patch_ids = []
                for img_id in validation[i:i+batch_size / 10]:
                    image = plt.imread(datapath + Path(image_paths[img_id]))
                    centers = np.load(samplepath + image_paths[img_id].split(".")[0] + ".npy").T
                    patches.extend(create_patches(image, centers, 80))
                    patch_ids.extend([img_id] * len(centers))
            ...
            sum_cost = 0.0
            sum_accuracy = 0.0
            test_length = len(test_images) - (len(test_images) % batch_size / 10)
            for i in range(0, test_length, batch_size / 10):
                patches = []
                patch_ids = []
                for img_id in test_images[i:i+batch_size / 10]:
                    image = plt.imread(datapath + Path(image_paths[img_id]))
                    centers = np.load(samplepath + image_paths[img_id].split(".")[0] + ".npy").T
                    patches.extend(create_patches(image, centers, 80))
                    patch_ids.extend([img_id] * len(centers))
